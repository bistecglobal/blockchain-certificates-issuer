...
export default function TrainerCardView({ trainersData }) {
    return (
        <>
            {trainersData.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {trainersData.map((trainer, index) => (
                    <div key={index} className="max-w-sm p-6 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <div>
                            <h5 className="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{trainer.Title}</h5>
                        </div>
             ...
             </div>) : (<div className='fixed inset-0 ml-40 flex items-center justify-center'></div>)
             }
        </>
    );
}