...
namespace AspNetCoreVerifiableCredentials
{
    ...
    public class IssuerController : ControllerBase
    {
        ...
        private IssuanceRequest SetClaims(IssuanceRequest request, Dictionary<string, string> data)
        {
            request.claims = new Dictionary<string, string>();
            request.claims.Add("course_name", data["course_name"]);
            request.claims.Add("date", data["date"]);
            request.claims.Add("trainee", data["trainee"]);
            request.claims.Add("trainer", data["trainer"]);

            string photoClaimName = "";
            // get photo claim from manifest (same logic as before)
            // ...

            if (!string.IsNullOrWhiteSpace(photoClaimName))
            {
                // logic to handle photo claim (same logic as before)
                // ...
            }
            return request;
        }
        ...
        public async Task<ActionResult> IssuanceRequest([FromBody] Dictionary<string, string> data)
        {
            _log.LogTrace(this.HttpContext.Request.GetDisplayUrl());
            try
            {
                // Validate input data
                if (data == null || !data.ContainsKey("course_name") || !data.ContainsKey("date") || !data.ContainsKey("trainer") || !data.ContainsKey("trainee"))
                {
                    return BadRequest(new { error = "400", error_description = "Missing required fields in request body." });
                }
